    ...
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          ...
          handleNewAddress({...addAddress,isAddingNewAddress:false});
          }}
        >
          Add
        </Button>
...
const Checkout = () => {
  ...
  const getProducts = async () => {
    try {
      ...
      if (e.response && e.response.status === 500) {
        enqueueSnackbar(e.response.data.message, { variant: "error" },{variant:'error'});
        return null;
      } else {
        enqueueSnackbar(
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      ...
      const response = await axios.post(url, {"address":newAddress.value}, { headers: { "Authorization": `Bearer ${localStorage.getItem('token')}`}});
      setAddresses({ ...addresses, all: response.data });
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      const response = await axios.post(url,{"addressId":addresses.selected},{ headers: { "Authorization": `Bearer ${localStorage.getItem('token')}`}});

      let newBalance=parseInt(localStorage.getItem('balance'),10);
      newBalance-=getTotalCartValue(items);
  ...
  const handleSelect=(e)=>{
    if(e===addresses.selected)
    setAddresses({ ...addresses, selected:""});

    else
    setAddresses({ ...addresses, selected: e});
  }
            ...
            <Box>
              ...
              {addresses &&
                addresses.all.map((add) => {
                  if (add._id === addresses.selected) {
                    return <Typography my="1rem" key={add._id} onClick={() => handleSelect(add._id)}

                      className="selected address-item"
                    >
                      {add.address}
                      <Button variant="contained" startIcon={<Delete />} onClick={async ()=>{
                        await deleteAddress(token,add._id)
                      }}>
                        Delete
                      </Button>
                    </Typography>
                    ...
                    return <Typography my="1rem" key={add._id} onClick={() => handleSelect(add._id)}
                      className="not-selected  address-item"
                    >
                      {add.address}
                      <Button variant="contained" startIcon={<Delete />} onClick={async ()=>{
                        await deleteAddress(token,add._id)
